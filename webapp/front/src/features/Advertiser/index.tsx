...
const daoContractAddress = '0x88198a63688A18347A993588ffDC1fdfbAd4800b';
...
export const Advertiser = () => {
  ...
  useEffect(() => {
    const loadWeb3 = async () => {
      if (address) {
        try {
          ...
          const _stakingADsTokenContract = getTokenContract(
            stakingADsTokenContractAddress,
            'token'
          );
          ...
          const _rewardADsAmount = await _daoContract.getRewardTokenInfo(
            rewardADsTokenContractAddress
          );
          const _rewardMeAmount = await _daoContract.getRewardTokenInfo(
            rewardMeTokenContractAddress
          );

          dispatch(setRewardADsAmount(_rewardADsAmount[0]));
          dispatch(setRewardMeAmount(_rewardMeAmount[0]));
          dispatch(
            setRewardADsSpeed(parseFloat(ethers.utils.formatEther(_rewardADsAmount[1])) * 60)
          );
    ...
    const initialApp = async () => {
      if (address || isStaking == false || isCreate == false) await loadWeb3();
    };
